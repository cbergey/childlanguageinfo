...
avg_chunks <- childes_htmm_pdwz %>%
  ...
  summarise(across(all_of(topic_probs), list(mean))) %>%
  ungroup() %>%
  mutate(age_bin = floor(target_child_age/6) *6) %>%
  arrange(transcript_id, chunk)
  
topic_probs <- paste0(topic_probs, "_1")
...
get_prob_t_matrix <- function(df) {
  ...
  parent_matrix <- map(parent_rows, 
    ...
    Reduce('+', .) %>% as.tibble() %>%
    mutate(probs_type = "parent") %>%
    cbind(topic_probs) 
  
  child_matrix <- map(child_rows, 
                      ~matrix_from_rows(df %>% slice(.x - 1), df %>% slice(.x))) %>%
    Reduce('+', .) %>% as.tibble() %>%
    mutate(probs_type = "child") %>%
    cbind(topic_probs) 
  
  return(bind_rows(parent_matrix, child_matrix) %>% rename(prior_topic = topic_probs))
}


chunks_nested_36 <- avg_chunks %>%
...
chunks_nested_24 %>%
  ...
  unnest(matrix) %>%
  group_by(age_bin, probs_type, current_topic) %>%
  summarise(across(all_of(topic_probs), list(mean))) %>%
  View()
