...
childes_htmm_pdwz <- childes_htmm_pdwz %>%
  ...
  mutate(second_lead_topic = lead(max_topic, n = 2),
         quadgram = paste(lag_topic, max_topic, lead_topic, second_lead_topic, sep = "_")) %>%
  ungroup()
...
snippet_tab <- childes_htmm_pdwz %>%
  ...
  rename("act_type" = "max_topic", "speaker" = "speaker_role") %>%
...
print(snippet_tab, type = "latex", comment = F, table.placement = "t", floating = TRUE,
      floating.environment = "table*",
      include.rownames = FALSE)
...
```{r branching_out_plot, fig.height = 3.4, fig.width=7, fig.align = "left", num.cols.cap=2, fig.cap = "Plot showing the distribution of topics produced by children and parents across development.", fig.pos="t"}
...
```{r entropy_plot, fig.height = 3.4, fig.width=7, fig.align = "left", num.cols.cap=2, fig.cap = "Entropy of topics produced by children, mothers and fathers over development.", fig.pos="b"}
topic_entropies %>%
  mutate(entropy_type = "Communicative Act") %>%
  rbind(word_entropies %>% mutate(entropy_type = "Productive Vocabulary")) %>%
...
```{r conditional_prob_plot, fig.env = "figure*", fig.width = 7, fig.height = 3}
...
print(cprobs_fig, fig.align = "center", fig.pos="b", 
      environment = "figure*")
