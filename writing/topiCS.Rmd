...
shorttitle        : "Classifying communicative acts in CHILDES"

author: 
  - name          : "Claire Bergey"
    affiliation   : "1"
    corresponding : yes    # Define only one corresponding author
    address       : "5848 S. University Avenue, Chicago, Illinois 60637"
    email         : "cbergey@uchicago.edu"
  - name          : "Zoe Marshall"
    affiliation   : "2"
    email         : "zmarshal@andrew.cmu.edu"
  - name          : "Simon DeDeo"
    affiliation   : "3,4"
...
authornote: |
  The authors thank Marisa Casillas and Thea Cameron-Faulkner for discussions about this work’s theoretical basis and Benjamin C. Morris, Ashley Leung, Megan Waller and Yuchen Zhou for feedback on the manuscript. The authors are grateful for discussion from the Language Evolution, Acquisition, and Processing group, the Goldin-Meadow lab, and the audience at the 43rd Annual Meeting of the Cognitive Science Society. This work was funded by the James S. McDonnell Foundation Scholar Award in Understanding Human Cognition to the last author and the Carnegie Mellon Summer Undergraduate Research Fellowship to the second author.
...
keywords          : "language acquisition, corpus analysis, computational modeling, pragmatics, discourse"
wordcount         : "4343"
...
To check that the 15-topic model was not capturing drastically different topic distinctions from other runs of the model, we calculated the mutual information between utterance-level topic classifications from the 15-topic model against the 10-topic and 20-topic models. These mutual information values were relatively high: between the 15- and 10-topic models, mutual information was 1.53 (upper bound: entropy of 10-topic model, 2.25) and between the 15- and 20-topic models, mutual information was 1.84 (upper bound: entropy of 15-topic model, 2.63). Thus, the model we chose captures similar information to alternative runs of the model, and is unlikely to represent a one-off set of distinctions. All data and code for analyses, as well as the corpus file with utterance classifications, are available at https://osf.io/wg6dc/ .

```{r topic-table, results="asis"}

topics <- c("mental states", "labeling", "counting", "evaluation", "proposed actions",
            "non-present events", "miscellaneous", "requests", "knowledge questions", 
            "location", "social routines", "backchannels",
            "description", "storytelling", "body routines")

topic_tab <- childes_htmm_phi %>%
  ...
  pivot_wider(names_from = num, names_prefix = "word_", values_from = word)  %>%
  mutate(topic_name = topics, topic = as.character(topic)) %>% select(topic, topic_name, everything()) %>%
  unite("distinctive_words", word_1:word_12, sep = ", ") %>%
  rename("Topic" = "topic", "Topic Name" = "topic_name", "Distinctive Words" = "distinctive_words")

# version of table as image
# img <- png::readPNG(here("writing/figs/topic-tab.png"))
# grid::grid.raster(img)

apa_table(topic_tab, font_size = "footnotesize",
         caption = "The most distinctive words in each topic. Distinctiveness is measured by the difference between a word's probability in a given topic and its average probability across all topics. Tokens representing utterances with all non-words are not included here.")
```
...
# print(snippet_tab, type = "latex", comment = F, 
#        floating = TRUE,
#        include.rownames = FALSE)

apa_table(snippet, font_size = "footnotesize",
         caption = "Three examples of the same communicative act sequence (5, 12, 4, 5) in conversations between different parents (P) and children (C) at different ages. Though they involve different topical content, they follow a similar communicative pattern: a suggestion or request regarding action and location; an affirmative or negative response; an evaluative statement or question; and another suggestion regarding action and location.")
```

To further validate our method, we considered sets of exchanges that share the same communicative act sequence but vary in their participants and content. Here and in Part 2, we consider patterns of usage when utterances are classified as their most probable act type. Table \ref{tab:snippet-table} shows three exchanges with the same act sequence between parents and their 23-, 39-, and 57-month-olds. Each exchange begins with a proposed action which receives a backchannel, the action is evaluated, and a new action is proposed. Across these three examples, the words in the utterances are different, and sometimes even of opposite valence (e.g., "okay" vs. "no no"). The role of the speaker also varies--the same act can be produced by both parents and children. Nonetheless, there is a structural resemblance across them. 
...
```{r branching-out-plot, fig.align = "left", fig.cap = "The proportion of utterances classified as each communicative act type produced by children and parents across development.", fig.width = 6, fig.height =3.5}
topics <- c("mental states", "labeling", "counting", "evaluation", "proposed actions",
...
branching_plot <- ggplot(branching, aes(x = age_bin, y = n, group = max_topic, 
             fill = as.factor(max_topic))) +
  geom_area(position = "fill", color = "black") +
  facet_wrap(~speaker_role) +
  theme_few(base_size = 10) +
...
Notably, children at the lowest end of our age range start with at least some representation of several communicative act types, and expand their repertoire rapidly. One way to characterize the diversity of communicative acts a person engages in is to measure the entropy of their communicative acts. Children's communicative act entropy increases drastically over development until around 24 months, at which point it matches parents' entropy and remains relatively stable across the rest of our age range, as shown in Figure \ref{fig:entropy-plot}. Our automatically-identified communicative acts are therefore capturing some capacity that becomes more adult-like across development. 

To characterize the learning process for these acts and compare it to word learning, we plot parents' and children's act entropies and the entropies of the words they produced in each transcript and fit a sigmoid function to children's trajectories (Figure \ref{fig:entropy-plot}; entropy values estimated using an implementation of the algorithm from @nemenman_entropy_2002). Words coded as babbling in the corpus were excluded from the measure of productive word use. We find that for both words and acts, children reach 90% of their final repertoire at roughly the same age, 24 months. Children's act entropy is growing most steeply at around 14 months, and their word entropy is growing most steeply at around 12 months. Meanwhile, parents are remarkably stable across all ages: parents provide a constant, high level of word and communicative act entropy. Prior work on speech acts has found that productive vocabulary relates to diversity of speech acts: @snow_learning_1996 found that the number of speech act types children use correlates highly with the number of word types they use. We replicate that finding here: children's number of communicative act types produced correlates with their word types produced, even within one-month age intervals (lowest correlation, r = 0.512, p < 0.001). 

Within these trajectories there is variability between individuals in their communicative act entropy. Parents' and children's individual act entropies are correlated (r = 0.583, p < 0.001), and when divided into one-month age intervals remain correlated in all groups 12 months and older (lowest correlation, r = 0.321, p = 0.010): parents who are using more diverse communicative acts are talking to children who are using more diverse communicative acts. 
...
```{r entropy-plot, out.width="70%", include=TRUE, fig.align="tb", fig.cap = "Entropy of communicative acts and words produced by children and parents over development.", echo = FALSE}
...
#ggsave(mi_plot, filename = "mi_plot.png",  width = 4, height = 3.5, units = "in", dpi = 300, bg = "transparent")
```
